...
class GuiManager():
    ...
    def __init__(self, master, config, queue, keyTrainer):
        ...
        self.buttonFont = tkFont.Font(
            family=config.font_name, size=config.font_size)
        self.boldUnderscoredButtonFont = tkFont.Font(
            family=config.font_name, size=config.font_size, weight='bold', underline=1)
        ...
        for row_index in xrange(1, config.getNumOfRows() + 1):
            ...
            for button_num in xrange(1, config.getNumOfKeysInRow(row_index) + 1):
                newButton = Button(
                    self.gui_rows[int(row_index)], fg='#cccccc', bg='#262626', relief=FLAT)
                ...
                if (row_index, int(button_num)) in config.key_pos_to_index:
                    self.gui_all_buttons[
                        config.key_pos_to_index[(row_index, int(button_num))]] = newButton
                self.gui_row_buttons[int(row_index)].append(newButton)
                newButton.pack(side=LEFT)

            self.gui_rows[int(row_index)].pack(fill=X, expand=True)
        ...
        if len(self.config.colored_keys) != 0:
            for button_index in self.config.colored_keys:
                if button_index in self.gui_all_buttons:
                    self.gui_all_buttons[button_index].configure(
                        bg=self.config.colored_keys[button_index], fg='#ccc')
    ...
    def mouse_entered(self, event):
        ...
        if not self.block_resizing:
            ...
            self.master.after(
                self.config.hide_timeout, self.resize_window_back)

    def reconfigure_text_on_buttons(self, config, shift_pressed, lang):
        for row_index in xrange(1, config.getNumOfRows() + 1):
            configured_buttons = dict(
                (k, v) for k, v in config.key_pos_to_index.items() if k[0] == row_index)
